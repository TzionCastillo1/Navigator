...
const int8_t wakePin = 10;        // MCU interrupt/alarm pin to wake from sleep
...
const float dividerGain =1;  //  Default 1/gain for grove voltage divider is 10x
// const uint8_t ADSi2c_addr = 0x48;  // The I2C address of the ADS1115 ADC
const uint8_t VoltReadsToAvg = 1;  // Only read one sample

ExternalVoltage extvolt(ADSPower, ADSChannel, dividerGain, 0x48, VoltReadsToAvg);
...
//create EZO Object
//AtlasScientificEC atlasEC(22, 0x64, 5);  //I2C for EC = 0x64; pH = 0x63
AtlasScientificpH atlaspH(22, 0x63, 5);
/*
float calculateVariableValue(void)
{
    float calculatedResult = -9999;  // Always safest to start with a bad value
      calculatedResult = -30.141*extvolt + 75.359;
    return calculatedResult;
}
Variable *calculatedVar = new Variable(calculateVariableValue);
*/
Variable *variableList[] = {
  ...
  //new Variable(calculateVariableValue)
  //new AtlasScientificEC_Cond(&atlasEC),
  new AtlasScientificpH_pH(&atlaspH),
  //new MaximDS3231_Temp(analogRead(A0))
